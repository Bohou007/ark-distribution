...
<div class="llms-lesson-preview<?php echo esc_attr( $lesson->get_preview_classes() ); ?>">
	<a class="llms-lesson-link<?php echo html_entity_decode( esc_html($restrictions['is_restricted'])) ? ' llms-lesson-link-locked' : ''; ?>" href="<?php echo ( ! $restrictions['is_restricted'] ) ? esc_url( get_permalink( $lesson->get( 'id' ) ) ) : '#llms-lesson-locked'; ?>"<?php echo html_entity_decode( esc_html($data_msg)); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
			...
			<?php if ( apply_filters( 'llms_display_outline_thumbnails', true ) ) : // phpcs:ignore WPThemeReview.CoreFunctionality.PrefixAllGlobals.NonPrefixedHooknameFound ?>
				<?php if ( has_post_thumbnail( $lesson->get( 'id' ) ) ) : ?>
					<div class="llms-lesson-thumbnail-wrap">
						<div class="llms-lesson-thumbnail post-thumbnail thebase-thumbnail-ratio-<?php echo esc_attr( thebase()->option( 'course_syllabus_thumbs_ratio', '2-3' ) ); ?>">
		...
		<div class="course-information">
			<?php if ( 'course' === get_post_type( get_the_ID() ) ) : ?>	
				<aside class="llms-extra">
					...
					<span class="llms-lesson-counter"><?php printf( _x( '%1$d of %2$d', 'lesson order within section', 'gadgeto' ), isset( $order ) ? $order : $lesson->get_order(), $total_lessons ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></span>
					<?php echo html_entity_decode( esc_html($lesson->get_preview_icon_html())); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
				</aside>
			...
			<section class="llms-main">
				<?php if ( 'lesson' === get_post_type( get_the_ID() ) ) : ?>
					<h6 class="llms-pre-text"><?php html_entity_decode( esc_html($pre_text)); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></h6>
				<?php endif; ?>
