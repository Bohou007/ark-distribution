<?php
/**
 * TheBase\Custom_Header\Component class
 *
 * @package thebase
 */

namespace TheBase\Custom_Header;
 ...
 * * `thebase()->render_header()`
 * * `thebase()->display_header_row()`
 * * `thebase()->has_center_column()`
 * * `thebase()->has_side_columns()`
 * * `thebase()->display_mobile_header_row()`
 * * `thebase()->has_mobile_center_column()`
 * * `thebase()->has_mobile_side_columns()`
 */
class Component implements Component_Interface, Templating_Component_Interface {
	...
	public function display_mobile_header_row( $row = 'main' ) {
		$display = false;
		foreach ( array( 'left', 'center', 'right' ) as $column ) {
			$elements = thebase()->option( 'header_mobile_items' );
			if ( isset( $elements ) && isset( $elements[ $row ] ) && isset( $elements[ $row ][ $row . '_' . $column ] ) && is_array( $elements[ $row ][ $row . '_' . $column ] ) && ! empty( $elements[ $row ][ $row . '_' . $column ] ) ) {
				$display = true;
				break;
			}
	...
	public function render_header( $row = 'main', $column = 'left', $header = 'desktop' ) {
		$elements = thebase()->option( 'header_' . $header . '_items' );
		if ( isset( $elements ) && isset( $elements[ $row ] ) && isset( $elements[ $row ][ $row . '_' . $column ] ) && is_array( $elements[ $row ][ $row . '_' . $column ] ) && ! empty( $elements[ $row ][ $row . '_' . $column ] ) ) {
			foreach ( $elements[ $row ][ $row . '_' . $column ] as $key => $item ) {
				$template = apply_filters( 'thebase_header_elements_template_path', 'template-parts/header/' . $item, $item, $row, $column );
