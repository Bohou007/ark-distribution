<?php
/**
 * TheBase\Nav_Menus\Component class
 *
 * @package thebase
 */

namespace TheBase\Nav_Menus;
 ...
 * * `thebase()->is_primary_nav_menu_active()`
 * * `thebase()->display_primary_nav_menu( array $args = [] )`
 * * `thebase()->display_fallback_menu( array $args = [] )`
 * * `thebase()->is_mobile_nav_menu_active( array $args = [] )`
 * * `thebase()->display_mobile_nav_menu( array $args = [] )`
 */
class Component implements Component_Interface, Templating_Component_Interface {
	...
	public function action_register_nav_menus() {
		register_nav_menus(
			array(
				static::PRIMARY_NAV_MENU_SLUG   => esc_html__( 'Primary', 'gadgeto' ),
				static::SECONDARY_NAV_MENU_SLUG => esc_html__( 'Secondary', 'gadgeto' ),
				static::MOBILE_NAV_MENU_SLUG    => esc_html__( 'Mobile', 'gadgeto' ),
				static::FOOTER_NAV_MENU_SLUG    => esc_html__( 'Footer', 'gadgeto' ),
			)
		);
	}
	...
	public function filter_primary_nav_menu_dropdown_symbol( $title, $item, $args, $depth ) {
		...
		if ( ! empty( $item->classes ) && in_array( 'menu-item-has-children', $item->classes ) ) {
			$title = '<span class="nav-drop-title-wrap">' . $title . '<span class="dropdown-nav-toggle">' . thebase()->get_icon( 'arrow-down' ) . '</span></span>';
		}
		//aria-label="' . esc_attr__( 'Expand child menu', 'gadgeto' ) . '"
	...
	public function filter_mobile_nav_menu_dropdown_symbol( $item_output, $item, $depth, $args ) {
		...
		if ( ! empty( $item->classes ) && in_array( 'menu-item-has-children', $item->classes ) ) {
			...
			$output = '<div class="drawer-nav-drop-wrap">' . $item_output . '<button class="drawer-sub-toggle" data-toggle-duration="10" data-toggle-target="' . esc_attr( $toggle_target_string ) . '" aria-expanded="false"><span class="screen-reader-text">' . esc_html__( 'Expand child menu', 'gadgeto' ) . '</span>' . thebase()->get_icon( 'arrow-down', '', false, false ) . '</button></div>';
			return $output;
		}
	...
	public function display_mobile_nav_menu( array $args = array() ) {
		...
		if ( ! isset( $args['mega_support'] ) && apply_filters( 'thebase_mobile_allow_mega_support', true ) ) {
			$args['mega_support'] = true;
		}
		$args['show_toggles']   = ( thebase()->option( 'mobile_navigation_collapse' ) ? true : false );
