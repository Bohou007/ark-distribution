<?php
/**
 * TheBase\Microdata\Component class
 *
 * @package thebase
 */

namespace TheBase\Microdata;
...
class Component implements Component_Interface, Templating_Component_Interface {
	...
	public function print_microdata( string $context ) {
		...
		if ( ! apply_filters( 'thebase_microdata', true, $context ) ) {
			return;
		}
	...
	public function get_microdata( $context ) {
		...
		if ( 'html' === $context ) {
			...
			$type = apply_filters( 'thebase_html_itemtype', $type );
		...
		if ( 'article' === $context ) {
			$type = apply_filters( 'thebase_article_itemtype', 'CreativeWork' );
		...
		if ( $data ) {
			return apply_filters( "thebase_{$context}_schema", $data );
		}
	}
}
