...
class Component implements Component_Interface {
	...
	public function initialize() {
		...
		// Hide Sale badge
		add_filter( 'woocommerce_show_product_sale_flash', array( $this, 'hide_sale_flash') );
		add_filter( 'woocommerce_sale_flash', array( $this, 'hide_sale_flash' ) );

		// Add Sale Percentage
		add_action( 'woocommerce_before_shop_loop_item_title', array( $this, 'sale_percentage' ),10);
		add_action( 'woocommerce_before_single_product_summary', array( $this, 'sale_percentage' ), 10 );
		...
		add_filter( 'thebase_blocks_carousel_woocommerce_product_loop_start', array( $this, 'product_loop_start' ), 5 );
		// Add Post grid class.
		...
		//Image wrap
		add_action( 'woocommerce_before_shop_loop_item_title', array( $this, 'archive_image_wrap_start' ),4  );
		add_action( 'woocommerce_before_shop_loop_item_title', array( $this, 'archive_image_wrap_end' ),51);			
		//Image wrap
		add_action( 'woocommerce_before_shop_loop_item', array( $this, 'archive_product_wrap_start' ),5  );
		add_action( 'woocommerce_after_shop_loop_item', array( $this, 'archive_product_wrap_end' ),50);	
		// Add Wrap to extra button.
		add_action( 'archive_image_wrap', array( $this, 'archive_extra_button_wrap_start' ),51  );
		add_action( 'archive_image_wrap', array( $this, 'archive_extra_button_wrap_end' ),51);
		remove_action( 'woocommerce_after_shop_loop_item', 'woocommerce_template_loop_add_to_cart' );
		add_action( 'archive_extra_button_wrap', 'woocommerce_template_loop_add_to_cart');
		add_action( 'archive_extra_button_wrap',  array( $this, 'quickview_in_product'));
		add_action( 'archive_extra_button_wrap',  array( $this,'compare_in_product') );	
		add_action( 'archive_extra_button_wrap', array( $this, 'wishlist_in_product'));					
		
		// Woocommerce rating stars always
		add_filter('woocommerce_product_get_rating_html',array( $this, 'get_rating_html'),10, 2);
		...
		add_action( 'thebase_before_main_content', array( $this, 'wc_print_notices_none_woo' ) );
		...
		add_action( 'thebase_before_sidebar_wrap', array( $this, 'close_shop_filter' ), 5 );

		//add_action( 'woocommerce_after_shop_loop_item_title', array( $this, 'woo_hot_product_countdown') ,15);
		add_action( 'woocommerce_after_shop_loop_item_title', array( $this, 'woo_archive_product_countdown') ,10);
		add_action( 'woocommerce_single_product_summary', array( $this, 'woo_product_countdown') ,5);	
		add_action( 'woocommerce_after_single_product_summary', array( $this, 'single_product_cms') ,5);

		//add_action( 'woocommerce_before_shop_loop_item_title', array( $this, 'woo_product_gallary') ,50);
	}
	/* Show an product count-down Counter */
	// public function woo_hot_product_countdown() {
	// 	if ( has_shortcode($post,'woo_hot_products')) {		
	...
	/* Show an product count-down Counter */
	public function woo_archive_product_countdown() {
		if ( thebase()->option( 'product_archive_countdown' ) ) {		
			global $post,$product , $today;
			$ids =  $product->get_id();
			$today = date('Y-m-d');
			$sale_price_dates_from = ( $date = get_post_meta( $post->ID, '_sale_price_dates_from', true ) ) ? date_i18n( 'Y-m-d', $date ) : '';
			...
			if ($today >= $sale_price_dates_from  && $today <= $sale_price_dates_to){
				if($product->is_type( 'simple' ) && $sale_price_dates_to != "" ) {	
				 // translators : Hurry Up! Limited Time
					echo '<div class="count-text">Hurry Up! Limited TIme</div>';
					echo '<div class="count-down"><div class="countbox hastime" data-time="'.$sale_price_dates_to.'"></div></div>';	
				}
			}	
		}	
	...
	public function woo_product_countdown() {
		if ( thebase()->option( 'product_countdown' ) ) {		
			global $post,$product , $today;
			$ids =  $product->get_id();
			$today = date('Y-m-d');
			$sale_price_dates_from = ( $date = get_post_meta( $post->ID, '_sale_price_dates_from', true ) ) ? date_i18n( 'Y-m-d', $date ) : '';
	...
	function single_product_cms(){
		echo '<div class="singleproduct-sidebar">';
		echo	'<div class="single-product-side-widget-area">';
		         dynamic_sidebar('single-product-side-widget-area'); 
				 echo   '</div> </div>';			
	}	
	...
	public function setup_shop_content_filter() {
		global $wp_embed;
		add_filter( 'thebase_theme_shop_content', array( $wp_embed, 'run_shortcode' ), 8 );
		add_filter( 'thebase_theme_shop_content', array( $wp_embed, 'autoembed'     ), 8 );
		add_filter( 'thebase_theme_shop_content', 'do_blocks' );
		add_filter( 'thebase_theme_shop_content', 'wptexturize' );
		add_filter( 'thebase_theme_shop_content', 'convert_chars' );
		add_filter( 'thebase_theme_shop_content', 'shortcode_unautop' );
		add_filter( 'thebase_theme_shop_content', 'do_shortcode', 11 );
		add_filter( 'thebase_theme_shop_content', 'convert_smilies', 20 );
	}	
	/**
		...
		if ( is_post_type_archive( 'product' ) && in_array( absint( get_query_var( 'paged' ) ), array( 0, 1 ), true ) ) {
			...
			if ( $shop_page ) {
				if ( has_blocks( $shop_page->post_content ) ) {
					$description = apply_filters( 'thebase_theme_shop_content', $shop_page->post_content );
				} else {
	...
	public function remove_product_tab_heading( $heading ) {
		if ( ! thebase()->option( 'product_tab_title' ) ) {
			return false;
		}
		return $heading;
	}
	...
	public function category_widget_toggle_script( $args ) {
		wp_enqueue_script( 'thebase-shop-toggle' );
		return $args;
	}
	/**
	...
	public function related_products_columns( $args ) {
		$columns = absint( thebase()->option( 'product_related_columns' ) );
	...
	public function custom_block_html( $html, $data, $product ) {
		...
		$product_btn_style = thebase()->option( 'product_archive_button_style' );
		$product_image_hover_style = thebase()->option( 'product_archive_image_hover_switch' );
		if ( 'button' === $product_btn_style ) {
			$cart_text = sprintf(
				'<a href="%s" %s>%s</a>',
				esc_url( $product->add_to_cart_url() ),
				wc_implode_html_attributes( $attributes ),
				esc_html( $product->add_to_cart_text() ) . ' ' . thebase()->get_icon( 'spinner' ) . ' ' . thebase()->get_icon( 'check' )
			);
		} else {
			$cart_text = sprintf(
				'<a href="%s" %s>%s</a>',
				esc_url( $product->add_to_cart_url() ),
				wc_implode_html_attributes( $attributes ),
				esc_html( $product->add_to_cart_text() ) . ' ' . thebase()->get_icon( 'arrow-right-alt' ) . ' ' . thebase()->get_icon( 'spinner' ) . ' ' . thebase()->get_icon( 'check' )
			);
		}
		...
		if ( thebase()->option( 'product_archive_button_align' ) ) {
			$align_archive_button = 'block-align-buttons-bottom';
		}
		$product_style     = thebase()->option( 'product_archive_style' );
		...
		$boxed = thebase()->option( 'product_archive_content_style' );
	...
	public function single_product_layout() {
		// Product Single Cat.
		$category_element = thebase()->option( 'product_content_element_category' );
		...
		// Product Rating.
		$rating_element = thebase()->option( 'product_content_element_rating' );
		if ( isset( $rating_element ) && is_array( $rating_element ) && false === $rating_element['enabled'] ) {
			remove_action( 'woocommerce_single_product_summary', 'woocommerce_template_single_rating' );
		}
		// Product Price.
		$price_element = thebase()->option( 'product_content_element_price' );
		...
		// Product Meta.
		$meta_element = thebase()->option( 'product_content_element_product_meta' );
		if ( isset( $meta_element ) && is_array( $meta_element ) && false === $meta_element['enabled'] ) {
			remove_action( 'woocommerce_single_product_summary', 'woocommerce_template_single_meta', 40 );
		}

		// Product Payments.
		$payments_element = thebase()->option( 'product_content_element_payments' );
		if ( isset( $payments_element ) && is_array( $payments_element ) && true === $payments_element['enabled'] ) {
			add_action( 'woocommerce_single_product_summary', array( $this, 'woocommerce_product_single_payments' ), 40);
		}
		// Product Sharing.
		$sharing_element = thebase()->option( 'product_content_element_sharing' );
		...
		if ( false === thebase()->option( 'product_additional_weight_dimensions' ) ) {
			add_filter( 'wc_product_enable_dimensions_display', '__return_false' );
		}
		// Related Products.
		if ( false === thebase()->option( 'product_related' ) ) {
	...
	public function woocommerce_product_single_extras() {
		...
		if ( isset( $extras_element ) && is_array( $extras_element ) && isset( $extras_element['title'] ) && ! empty( $extras_element['title'] ) ) {
			echo '<p><span>' . wp_kses_post( $extras_element['title'] ) . '</span></p>';
		}
		echo '<ul>';
		if ( isset( $extras_element ) && is_array( $extras_element ) && isset( $extras_element['feature_1'] ) && ! empty( $extras_element['feature_1'] ) ) {
			echo '<li>' . thebase()->get_icon( isset( $extras_element['feature_1_icon'] ) && ! empty( $extras_element['feature_1_icon'] ) ? $extras_element['feature_1_icon'] : 'shield_check' ) . ' ' . wp_kses_post( $extras_element['feature_1'] ) . '</li>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
		if ( isset( $extras_element ) && is_array( $extras_element ) && isset( $extras_element['feature_2'] ) && ! empty( $extras_element['feature_2'] ) ) {
			echo '<li>' . thebase()->get_icon( isset( $extras_element['feature_2_icon'] ) && ! empty( $extras_element['feature_2_icon'] ) ? $extras_element['feature_2_icon'] : 'shield_check' ) . ' ' . wp_kses_post( $extras_element['feature_2'] ) . '</li>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
		if ( isset( $extras_element ) && is_array( $extras_element ) && isset( $extras_element['feature_3'] ) && ! empty( $extras_element['feature_3'] ) ) {
			echo '<li>' . thebase()->get_icon( isset( $extras_element['feature_3_icon'] ) && ! empty( $extras_element['feature_3_icon'] ) ? $extras_element['feature_3_icon'] : 'shield_check' ) . ' ' . wp_kses_post( $extras_element['feature_3'] ) . '</li>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
		if ( isset( $extras_element ) && is_array( $extras_element ) && isset( $extras_element['feature_4'] ) && ! empty( $extras_element['feature_4'] ) ) {
			echo '<li>' . thebase()->get_icon( isset( $extras_element['feature_4_icon'] ) && ! empty( $extras_element['feature_4_icon'] ) ? $extras_element['feature_4_icon'] : 'shield_check' ) . ' ' . wp_kses_post( $extras_element['feature_4'] ) . '</li>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
		if ( isset( $extras_element ) && is_array( $extras_element ) && isset( $extras_element['feature_5'] ) && ! empty( $extras_element['feature_5'] ) ) {
			echo '<li>' . thebase()->get_icon( isset( $extras_element['feature_5_icon'] ) && ! empty( $extras_element['feature_5_icon'] ) ? $extras_element['feature_5_icon'] : 'shield_check' ) . ' ' . wp_kses_post( $extras_element['feature_5'] ) . '</li>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
		echo '</ul>';
		echo '</div>';
	}
	...
	public function woocommerce_product_single_payments() {
		...
		if ( isset( $payments_element ) && is_array( $payments_element ) && isset( $payments_element['stripe'] ) && true === $payments_element['stripe'] ) {
			echo '<li class="single-product-payments-stripe">' . thebase()->get_icon( 'stripe' ) . '</li>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
		if ( isset( $payments_element ) && is_array( $payments_element ) && isset( $payments_element['visa'] ) && true === $payments_element['visa'] ) {
			echo '<li class="single-product-payments-visa">' . thebase()->get_icon( ( 'inherit' !== $colors ? 'visa_gray' : 'visa' ) ) . '</li>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
		if ( isset( $payments_element ) && is_array( $payments_element ) && isset( $payments_element['mastercard'] ) && true === $payments_element['mastercard'] ) {
			echo '<li class="single-product-payments-mastercard">' . thebase()->get_icon( ( 'inherit' !== $colors ? 'mastercard_gray' : 'mastercard' ) ) . '</li>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
		if ( isset( $payments_element ) && is_array( $payments_element ) && isset( $payments_element['amex'] ) && true === $payments_element['amex'] ) {
			echo '<li class="single-product-payments-amex">' . thebase()->get_icon( ( 'inherit' !== $colors ? 'amex_gray' : 'amex' ) ) . '</li>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
		if ( isset( $payments_element ) && is_array( $payments_element ) && isset( $payments_element['discover'] ) && true === $payments_element['discover'] ) {
			echo '<li class="single-product-payments-discover">' . thebase()->get_icon( ( 'inherit' !== $colors ? 'discover_gray' : 'discover' ) ) . '</li>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
		if ( isset( $payments_element ) && is_array( $payments_element ) && isset( $payments_element['paypal'] ) && true === $payments_element['paypal'] ) {
			echo '<li class="single-product-payments-paypal">' . thebase()->get_icon( ( 'inherit' !== $colors ? 'paypal_gray' : 'paypal' ) ) . '</li>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
		if ( isset( $payments_element ) && is_array( $payments_element ) && isset( $payments_element['applepay'] ) && true === $payments_element['applepay'] ) {
			echo '<li class="single-product-payments-applepay">' . thebase()->get_icon( ( 'inherit' !== $colors ? 'applepay_gray' : 'applepay' ) ) . '</li>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
	...
	public function woo_extra_body_classes( $classes ) {
		...
		if ( is_store_notice_showing() ) {
			$placement = thebase()->option( 'woo_store_notice_placement' );
			$classes[] = esc_attr( 'thebase-store-notice-placement-' . $placement );
			if ( 'above' === $placement ) {
				if ( thebase()->option( 'woo_store_notice_hide_dismiss' ) ) {
					add_filter( 'woocommerce_demo_store', array( $this, 'woocommerce_demo_store_remove_dismiss' ), 15, 2 );
				}
				remove_action( 'wp_footer', 'woocommerce_demo_store' );
				add_action( 'thebase_before_header', 'woocommerce_demo_store' );								
			}
		}
	...
	public function woocommerce_demo_store_remove_dismiss( $notice_html, $notice ) {
		...
		return '<p class="woocommerce-store-notice woo-static-store-notice demo_store" data-notice-id="' . esc_attr( $notice_id ) . '" style="display:block;">' . esc_html( $notice ) . '</p>';		
	}
	...
	public function my_account_body_classes( array $classes ) : array {
		if ( is_account_page() ) {
			$classes[] = 'thebase-account-nav-' . esc_attr( thebase()->option( 'woo_account_navigation_layout' ) );
		}
		return $classes;
	}
	/**
	...
	public function single_product_body_classes( array $classes ) : array {
		if ( is_product() ) {
			$cart_element = thebase()->option( 'product_content_element_add_to_cart' );
			...
			} else if ( thebase()->option( 'product_large_cart_button' ) ) {
				$size = 'large';
			} else {
				$size = 'normal';
			}
			$classes[] = 'product-tab-style-' . esc_attr( thebase()->option( 'product_tab_style' ) );
			$classes[] = 'product-variation-style-' . esc_attr( thebase()->option( 'variation_direction' ) );
			$classes[] = 'thebase-cart-button-' . esc_attr( $size );
		}
		return $classes;
	}
	/**
	...
	public function get_refreshed_fragments_mini( $fragments ) {
		...
		echo '<div class="thebase-mini-cart-refresh">';
		woocommerce_mini_cart();
		echo '</div>';
		$fragments['div.thebase-mini-cart-refresh'] = ob_get_clean();
	...
	public function check_for_fragment_support() {
		...
		if ( thebase()->option( 'header_cart_show_total' ) ) {
			add_filter( 'woocommerce_add_to_cart_fragments', array( $this, 'get_refreshed_fragments_number' ) );
		}
		if ( 'slide' === thebase()->option( 'header_cart_style' ) || 'slide' === thebase()->option( 'header_mobile_cart_style' ) || 'dropdown' === thebase()->option( 'header_cart_style' ) ) {
	...
	public function cart_in_header() {
		$in_header = false;
		$elements  = thebase()->option( 'header_desktop_items' );
	...
	public function action_enqueue_product_scripts() {
		if ( class_exists( 'woocommerce' ) && is_product() ) {
			...
			// wp_script_add_data( 'thebase-product-cls', 'async', true );
			// wp_script_add_data( 'thebase-product-cls', 'precache', true );
		}
	}
	/**
	...
	public function action_enqueue_scripts() {
		...
		if ( thebase()->option( 'custom_quantity' ) ) {
			...
			wp_enqueue_script(
				'thebase-shop-spinner',
				get_theme_file_uri( '/assets/js/shop-spinner.min.js' ),
				array( 'jquery' ),
				gadgeto_VERSION,
				true
			);
			wp_script_add_data( 'thebase-shop-spinner', 'async', true );
			wp_script_add_data( 'thebase-shop-spinner', 'precache', true );
		}
		if ( class_exists( 'woocommerce' ) && is_cart() ) {
			...
			wp_script_add_data( 'thebase-cart-update', 'async', true );
			wp_script_add_data( 'thebase-cart-update', 'precache', true );
		}
		...
		wp_script_add_data( 'thebase-shop-toggle', 'async', true );
		wp_script_add_data( 'thebase-shop-toggle', 'precache', true );
		wp_localize_script(
			'thebase-shop-toggle',
			'thebaseShopConfig',
			array(
	...
	public function wc_print_notices_none_woo() {
		if ( ! is_shop() && ! is_woocommerce() && ! is_cart() && ! is_checkout() && ! is_account_page() ) {
			echo '<div class="thebase-woo-messages-none-woo-pages woocommerce-notices-wrapper">';
	...
	public function cart_summary_title() {
		echo '<div class="cart-summary"><h2>' . esc_html__( 'Cart Summary', 'gadgeto' ) . '</h2></div>';
	}
	/**
	 * Insert the Cart Form wrap.
	 */
	public function cart_form_wrap_before() {
		echo '<div class="thebase-woo-cart-form-wrap">';
	}
	/**
	...
	public function myaccount_nav_avatar() {
		...
		if ( thebase()->option( 'woo_account_navigation_avatar' ) && 0 !== $current_user->ID ) {
			?>
			<div class="thebase-account-avatar">
				<div class="thebase-customer-image">
				<a class="tb-link-to-gravatar" href="https://gravatar.com/" target="_blank" rel="no" title="<?php echo esc_attr__( 'Update Profile Photo', 'gadgeto' ); ?>">
					<?php echo get_avatar( $current_user->ID, 60 ); ?>
				</a>
				</div>
				<div class="thebase-customer-name">
	...
	public function product_loop_start( $output ) {
		...
		elseif ( 7 === $columns ) {
			$columns_class = 'content-wrap product-archive grid-cols grid-ss-col-2 grid-sm-col-3 grid-md-col-4 grid-lg-col-6 grid-lg-col-7';
		}
		$main_archive_loop = '';
		if ( is_main_query() && is_archive() && ! wc_get_loop_prop( 'is_shortcode' ) ) {
		...
		$product_btn_style = thebase()->option( 'product_archive_button_style' );
		$product_image_hover_style = thebase()->option( 'product_archive_image_hover_switch' );
		if ( is_main_query() && is_archive() && wc_get_loop_prop( 'is_paginated' ) && apply_filters( 'thebase_enabled_product_archive_attributes', true, $GLOBALS['woocommerce_loop'] ) ) {
	...
	public function get_archive_infinite_attributes() {
		$attributes = '';
		return apply_filters( 'thebase_product_archive_infinite_attributes', $attributes );
	}
	/**
	...
	public function archive_loop_image_link_open() {
		...
		$has_hover_image = '';
		if ( 'none' !== thebase()->option( 'product_archive_image_hover_switch' ) ) {
			if ( is_a( $product, 'WC_Product' ) ) {
				$attachment_ids = $product->get_gallery_image_ids();
				if ( $attachment_ids ) {
	...
	public function archive_excerpt() {
		if ( is_main_query() && is_archive() ) {
			...
			if ( 1 === $columns || thebase()->option( 'product_archive_toggle' ) ) {
				global $post;
	...
	/**
	 * Start Action Extra Buttton Wrap.
	 */
	public function archive_extra_button_wrap_start() {
	 ...
	 */
	public function archive_extra_button_wrap_end() {
		echo '</div>'; 
	}

	/* To display Wishlist in product block */
	public function wishlist_in_product(){
	...
	/* quick view block */
	public function quickview_in_product(){
	...
	/* Compare Button */
	public function compare_in_product(){
	 ...
	 * Start Action Extra Buttton Wrap.
	 */
	public function archive_image_wrap_start() {
	 ...
	 * Close Action Extra buttons wrap.
	 */
	public function archive_image_wrap_end() {
		echo '</div>'; 
	}
	/**
	 * Start Action Extra Buttton Wrap.
	 */
	public function archive_product_wrap_start() {
		echo '<div class="container-inner">';
	}
	/**
	 * Close Action Extra buttons wrap.
	 */
	public function archive_product_wrap_end() {
		echo '</div>'; 
	}
	function get_rating_html($rating_html, $rating)
	{
		...
		$title = sprintf(
			// translators: %s: Product rating.
				__( 'Rated %s out of 5', 'gadgeto' ), $rating );
		} else {
		$title = 'Not yet rated';
		$rating = 0;
		}
		$rating_html  = '<div class="star-rating" title="' . $title . '">';
		$rating_html .= '<span style="width:' . (($rating / 5) * 100) . '%"><strong class="rating">' . $rating . '</strong> ' . esc_attr__('out of 5', 'gadgeto') . '</span>';
		$rating_html .= '</div>';		
		return $rating_html;
		}
	/**
	...
	public function archive_loop_top() {
		...
		if ( thebase()->option( 'product_archive_show_results_count' ) || thebase()->option( 'product_archive_show_order' ) || thebase()->option( 'product_archive_toggle' ) || apply_filters( 'thebase_product_archive_show_top_row', false ) ) {
			echo '<div class="thebase-shop-top-row">';
			do_action( 'thebase_woocommerce_before_shop_loop_top_row' );
			if ( thebase()->option( 'product_archive_toggle' ) ) {
				echo '<div class="thebase-shop-top-item thebase-woo-toggle">';
					$this->toggle_list();
				echo '</div>';
			}
			if ( thebase()->option( 'product_archive_show_results_count' ) ) {
				echo '<div class="thebase-shop-top-item thebase-woo-results-count">';
					woocommerce_result_count();
				echo '</div>';
			}
			if ( thebase()->option( 'product_archive_show_order' ) ) {
				echo '<div class="thebase-shop-top-item thebase-woo-ordering">';
					woocommerce_catalog_ordering();
				echo '</div>';
			}
			do_action( 'thebase_woocommerce_after_shop_loop_top_row' );
			echo '</div>';
		}
	}
	...
	public function shop_filter() {
		...
		echo '<span class="menu-toggle-icon">'.thebase()->print_icon( 'menu', '', false ).''.esc_html_e( 'Show Sidebar', 'gadgeto' ).'</span>';
		echo '</div>';
		}
	}
	/**
	...
	/**
	 * Hide Sale Percentage
	 */
	public function hide_sale_flash($sale) {
		if ( thebase()->option( 'product_archive_hide_sale_badge' ) ) {
			return false;
		}
		return $sale;
	}
	...
	/**
	 * Adds Sale Percentage
	 */
	public function sale_percentage() {
		if ( thebase()->option( 'product_archive_show_sale_percentage_badge' ) ) {
			global $product;
			$max_percentage = 0;
	...
	public function toggle_list() {
		...
		if ( 1 === wc_get_loop_prop( 'columns' ) ) {
			echo '<div class="thebase-product-toggle-container thebase-product-toggle-outer thebase-single-to-grid">';
				echo '<button title="' . esc_attr__( 'List View', 'gadgeto' ) . '" class="thebase-toggle-shop-layout thebase-toggle-list toggle-active" data-archive-toggle="list">';
					thebase()->print_icon( 'list', '', false );
				echo '</button>';
				echo '<button title="' . esc_attr__( 'Grid View', 'gadgeto' ) . '" class="thebase-toggle-shop-layout thebase-toggle-grid" data-archive-toggle="grid">';
					thebase()->print_icon( 'grid', '', false );
				echo '</button>';
			echo '</div>';
		} else {
			echo '<div class="thebase-product-toggle-container thebase-product-toggle-outer">';
				echo '<button title="' . esc_attr__( 'Grid View', 'gadgeto' ) . '" class="thebase-toggle-shop-layout thebase-toggle-grid toggle-active" data-archive-toggle="grid">';
					thebase()->print_icon( 'grid', '', false );
				echo '</button>';
				echo '<button title="' . esc_attr__( 'List View', 'gadgeto' ) . '" class="thebase-toggle-shop-layout thebase-toggle-list" data-archive-toggle="list">';
					thebase()->print_icon( 'list', '', false );
				echo '</button>';
			echo '</div>';
		}
	...
	public function output_product_above() {
		if ( is_product() && 'breadcrumbs' === thebase()->option( 'product_above_layout' ) ) {
	...
	public function output_product_above_title() {
		if ( is_product() && 'title' === thebase()->option( 'product_above_layout' ) ) {
	...
	public function output_content_wrapper() {
		...
		if ( ! is_product() && thebase()->show_in_content_title() ) {
