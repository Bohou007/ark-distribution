<?php
/**
 * TheBase\LearnDash\Component class
 *
 * @package thebase
 */

namespace TheBase\LearnDash;
...
class Component implements Component_Interface {
	...
	public function learndash_course_grid_class( $output, $atts, $filter ) {
		if ( thebase()->option( 'learndash_course_grid' ) ) {
	...
	public function learndash_styles() {
		wp_enqueue_style( 'thebase-learndash', get_theme_file_uri( '/assets/css/learndash.min.css' ), array(), gadgeto_VERSION );
		if ( class_exists( 'LearnDash_Settings_Section' ) && apply_filters( 'thebase_learndash_colors', true ) && ! defined( 'LDX_DESIGN_UPGRADE_PRO_LEARNDASH_VERSION' ) ) {
			$colors = array(
			...
			if ( isset( $colors['primary'] ) && empty( $colors['primary'] ) ) {
				...
				*/
				...
				.learndash-wrapper .ld-focus-comment.role-administrator>.ld-comment-wrapper {
					background-color: <?php echo esc_attr( $this->learndash_hex2rgb( thebase()->palette_option( 'palette1' ), '0.03' ) ); ?> !important;
				}
				...
				.learndash-wrapper .ld-status.ld-status-unlocked {
					background-color: <?php echo esc_attr( $this->learndash_hex2rgb( thebase()->palette_option( 'palette1' ), '0.2' ) ); ?> !important;
					color: var(--global-palette1) !important;
				}

				.learndash-wrapper .wpProQuiz_content .wpProQuiz_addToplist {
					background-color: <?php echo esc_attr( $this->learndash_hex2rgb( thebase()->palette_option( 'palette1' ), '0.1' ) ); ?> !important;
				...
				if ( ! empty( $custom_css ) ) {
					wp_add_inline_style( 'thebase-learndash', $custom_css );
				}
			}
		}
	}
