...
class Component implements Component_Interface, Templating_Component_Interface {
	...
	public function wp_starter_templates_notice() {
		...
		<div id="thebase-notice-starter-templates" class="notice is-dismissible notice-info">
			<h2 class="notice-title"><?php echo esc_html__( 'Thanks for choosing the gadgeto Theme!', 'gadgeto' ); ?></h2>
			<p class="thebase-notice-description"><?php /* translators: %s: <strong> <a> */ printf( esc_html__( 'Want to get started with a beautiful %1$sstarter template%2$s? Install the WP Starter Templates plugin to launch an optimized design in minutes.', 'gadgeto' ), '<a href="https://wordpress.org/plugins/wp-starter-templates/" target="_blank">', '</a>', '<strong>', '</strong>' ); ?></p>
			<p class="install-submit">
				<button class="button button-primary thebase-install-starter-btn" data-redirect-url="<?php echo esc_url( admin_url( 'themes.php?page=wp-starter-templates' ) ); ?>" data-activating-label="<?php echo esc_attr__( 'Activating...', 'gadgeto' ); ?>" data-activated-label="<?php echo esc_attr__( 'Activated', 'gadgeto' ); ?>" data-installing-label="<?php echo esc_attr__( 'Installing...', 'gadgeto' ); ?>" data-installed-label="<?php echo esc_attr__( 'Installed', 'gadgeto' ); ?>" data-action="<?php echo esc_attr( $data_action ); ?>"><?php echo esc_html( $button_label ); ?></button>
			</p>
		</div>
		<?php
	...
	public function thebase_turn_off_gutenberg_plugin_notice() {
		...
		<div id="thebase-notice-gutenberg-plugin" class="notice is-dismissible notice-warning">
			<h2 class="notice-title"><?php echo esc_html__( 'Gutenberg Plugin Detected', 'gadgeto' ); ?></h2>
			<p class="thebase-notice-description"><?php /* translators: %s: <a> */ printf( esc_html__( 'The %1$sGutenberg plugin%2$s is not recommended for use in a production site. Many things may be broken by using this plugin. Please deactivate.', 'gadgeto' ), '<a href="https://wordpress.org/plugins/gutenberg/" target="_blank">', '</a>' ); ?></p>
		</div>
		<?php
	...
	public function install_starter_script() {
		wp_register_script( 'wp-starter-install', get_template_directory_uri() . '/assets/js/admin-activate.min.js', array( 'jquery' ), gadgeto_VERSION, false );
		wp_register_script( 'thebase-gutenberg-deactivate', get_template_directory_uri() . '/assets/js/gutenberg-notice.min.js', array( 'jquery' ), gadgeto_VERSION, false );
	}
	/**
	 * Active Plugin Check
	 *
	...
	public function install_plugin_ajax_callback() {
		if ( ! check_ajax_referer( 'thebase-ajax-verification', 'security', false ) ) {
			wp_send_json_error( __( 'Security Error, Please reload the page.', 'gadgeto' ) );
		}
		if ( ! current_user_can( 'install_plugins' ) ) {
			wp_send_json_error( __( 'Security Error, Need higher Permissions to install plugin.', 'gadgeto' ) );
		}
		...
		if ( false === $install ) {
			wp_send_json_error( __( 'Error, plugin could not be installed, please install manually.', 'gadgeto' ) );
		} else {
