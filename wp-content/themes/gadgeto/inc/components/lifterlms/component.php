<?php
/**
 * TheBase\LifterLMS\Component class
 *
 * @package thebase
 */

namespace TheBase\LifterLMS;
...
class Component implements Component_Interface {
	...
	public function dashboard_wrapper_open() {
		echo '<div class="thebase-llms-dash-wrap thebase-llms-dash-nav-' . esc_attr( thebase()->option( 'llms_dashboard_navigation_layout' ) ) . '">';
	}
	/**
	...
	public function lifter_syllabus_thumbnails( $show ) {
		if ( thebase()->option( 'course_syllabus_thumbs' ) ) {
			$show = true;
		} else {
			$show = false;
		}
	...
	public function set_lifter_columns( $columns ) {
		...
		if ( get_the_ID() === $dash_id ) {
			$columns = absint( thebase()->option( 'llms_dashboard_archive_columns' ) );
		} elseif ( is_archive() ) {
			if ( is_post_type_archive( 'course' ) || is_tax( 'course_cat' ) || is_tax( 'course_tag' ) || is_tax( 'course_track' ) ) {
				$columns = absint( thebase()->option( 'course_archive_columns' ) );
			} elseif ( is_post_type_archive( 'llms_membership' ) || is_tax( 'membership_cat' ) || is_tax( 'membership_tag' ) ) {
				$columns = absint( thebase()->option( 'llms_membership_archive_columns' ) );
			}
		}
		return $columns;
	}
	...
	public function lifterlms_styles() {
		wp_enqueue_style( 'thebase-lifterlms', get_theme_file_uri( '/assets/css/lifterlms.min.css' ), array(), gadgeto_VERSION );
	}
