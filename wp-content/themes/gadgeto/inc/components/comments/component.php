<?php
/**
 * TheBase\Comments\Component class
 *
 * @package thebase
 */

namespace TheBase\Comments;
 ...
 * * `thebase()->the_comments( array $args = [] )`
 *
...
class Component implements Component_Interface, Templating_Component_Interface {
	...
	public function the_comments( array $args = array() ) {
		...
		$amp_live_list = thebase()->using_amp_live_list_comments();
		...
		?>
			<ol class="comment-list"<?php echo html_entity_decode( esc_html($amp_live_list)) ? ' items' : ''; ?>>
		...
		if ( $amp_live_list ) {
			...
			?>
				<div update>
					<button class="button" on="tap:amp-live-comments-list-<?php the_ID(); ?>.update"><?php esc_html_e( 'New comment(s)', 'gadgeto' ); ?></button>
				</div>
			</amp-live-list>
			<?php
		}
	...
	public function filter_default_fields_markup( $fields ) {
		...
		$show_web  = thebase()->option( 'comment_form_remove_web' );
		$fields['author'] = '<div class="comment-input-wrap ' . ( $show_web ? 'no-url-field' : 'has-url-field' ) . '"><p class="comment-form-author"><input aria-label="' . esc_attr__( 'Name', 'gadgeto' ) . '" id="author" name="author" type="text" placeholder="' . esc_attr__( 'John Doe', 'gadgeto' ) . '" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30" maxlength="245"' . $aria_req . ' /><label class="float-label" for="author">' . esc_html__( 'Name', 'gadgeto' ) . $label_req . '</label></p>';

		$fields['email'] = '<p class="comment-form-email"><input aria-label="' . esc_attr__( 'Email', 'gadgeto' ) . '" id="email" name="email" type="email" placeholder="' . esc_attr__( 'john@example.com', 'gadgeto' ) . '" value="' . esc_attr( $commenter['comment_author_email'] ) . '" size="30" maxlength="100" aria-describedby="email-notes"' . $aria_req . ' /><label class="float-label" for="email">' . esc_html__( 'Email', 'gadgeto' ) . $label_req . '</label></p>';
		if ( $show_web ) {
			$fields['url'] = '</div>';
		} else {
			$fields['url'] = '<p class="comment-form-url"><input aria-label="' . esc_attr__( 'Website', 'gadgeto' ) . '" id="url" name="url" type="url" placeholder="' . esc_attr__( 'https://www.example.com', 'gadgeto' ) . '" value="' . esc_attr( $commenter['comment_author_url'] ) . '" size="30" maxlength="200" /><label class="float-label" for="url">' . esc_html__( 'Website', 'gadgeto' ) . '</label></p></div>';
		}

		return apply_filters( 'thebase_comment_fields', $fields );
	}
	...
	public function filter_default_markup( $args ) {
		...
		$args['comment_field'] = '<p class="comment-form-comment comment-form-float-label"><textarea id="comment" name="comment" placeholder="' . esc_attr__( 'Leave a comment...', 'gadgeto' ) . '" cols="45" rows="8" maxlength="65525" aria-required="true" required="required"></textarea><label class="float-label" for="comment">' . esc_html__( 'Comment', 'gadgeto' ) . ' <span class="required">*</span></label></p>';

		return apply_filters( 'thebase_comment_args', $args );
	}
