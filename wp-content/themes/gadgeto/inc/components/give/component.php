<?php
/**
 * TheBase\GiveWP\Component class
 *
 * @package thebase
 */

namespace TheBase\Give;
...
class Component implements Component_Interface {
	...
	public function maybe_add_title() {
		if ( function_exists( 'give_get_meta' ) ) {
			...
			if ( ( ! $form_template || 'legacy' === $form_template ) && thebase()->show_in_content_title() ) {
				get_template_part( 'template-parts/content/entry_header', get_post_type() );
			}
		} elseif ( thebase()->show_in_content_title() ) {
			get_template_part( 'template-parts/content/entry_header', get_post_type() );
		}
	}
	/**
	...
	public function give_styles() {
		wp_enqueue_style( 'thebase-givewp', get_theme_file_uri( '/assets/css/givewp.min.css' ), array(), gadgeto_VERSION );
	}
	/**
	 * Adds theme output Wrapper.
	 */
	...
	public function add_iframe_fonts() {
		...
		$google_fonts = apply_filters( 'thebase_theme_givewp_google_fonts_array', self::$google_fonts );
		...
		if ( apply_filters( 'thebase_givewp_display_swap_google_fonts', true ) ) {
			$args['display'] = 'swap';
		}
		$font_url = add_query_arg( apply_filters( 'thebase_theme_givewp_google_fonts_query_args', $args ), 'https://fonts.googleapis.com/css' );
		if ( ! empty( $font_url ) ) {
			wp_enqueue_style( 'thebase-givewp-iframe-fonts', $font_url, 'give-sequoia-template-css' ); // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion
		}
	}
	/**
	...
	public function override_iframe_template_styles() {
		...
		$css->add_property( '--global-palette-highlight', $css->render_color( thebase()->sub_option( 'link_color', 'highlight' ) ) );
		$css->add_property( '--global-palette-highlight-alt', $css->render_color( thebase()->sub_option( 'link_color', 'highlight-alt' ) ) );
		$css->add_property( '--global-palette-highlight-alt2', $css->render_color( thebase()->sub_option( 'link_color', 'highlight-alt2' ) ) );
		$css->add_property( '--global-palette-btn-bg', $css->render_color( thebase()->sub_option( 'buttons_background', 'color' ) ) );
		$css->add_property( '--global-palette-btn-bg-hover', $css->render_color( thebase()->sub_option( 'buttons_background', 'hover' ) ) );
		$css->add_property( '--global-palette-btn', $css->render_color( thebase()->sub_option( 'buttons_color', 'color' ) ) );
		$css->add_property( '--global-palette-btn-hover', $css->render_color( thebase()->sub_option( 'buttons_color', 'hover' ) ) );
