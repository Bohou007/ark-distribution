...
function footer_navigation() {
	?>
	<nav id="footer-navigation" class="footer-navigation"  aria-label="<?php esc_attr_e( 'Footer Navigation', 'gadgeto' ); ?>">
	...
	if ( is_array( $items ) && ! empty( $items ) ) {
		foreach ( $items as $item ) {
			if ( $item['enabled'] ) {
				...
				echo '<a href="' . esc_attr( $link ) . '"' . ( $show_label ? '' : ' aria-label="' . esc_attr( $item['label'] ) . '"' ) . ' ' . ( 'phone' === $item['id'] || 'email' === $item['id'] || apply_filters( 'thebase_social_link_target', false, $item ) ? '' : 'target="_blank" rel="noopener noreferrer"  ' ) . 'class="social-button footer-social-item social-link-' . esc_attr( $item['id'] ) . esc_attr( 'image' === $item['source'] ? ' has-custom-image' : '' ) . '">';
...
function scroll_up() {
	if ( thebase()->option( 'scroll_up' ) ) {
		echo '<a id="tb-scroll-up" tabindex="-1" aria-hidden="true" aria-label="' . esc_attr__( 'Scroll to top', 'gadgeto' ) . '" href="#wrapper" class="thebase-scroll-to-top scroll-up-wrap scroll-ignore scroll-up-side-' . esc_attr( thebase()->option( 'scroll_up_side' ) ) . ' scroll-up-style-' . esc_attr( thebase()->option( 'scroll_up_style' ) ) . ' vs-lg-' . ( thebase()->sub_option( 'scroll_up_visiblity', 'desktop' ) ? 'true' : 'false' ) . ' vs-md-' . ( thebase()->sub_option( 'scroll_up_visiblity', 'tablet' ) ? 'true' : 'false' ) . ' vs-sm-' . ( thebase()->sub_option( 'scroll_up_visiblity', 'mobile' ) ? 'true' : 'false' ) . '">';
		thebase()->print_icon( thebase()->option( 'scroll_up_icon' ), esc_attr__( 'Scroll to top', 'gadgeto' ), false );
		echo '</a>';
		echo '<button id="tb-scroll-up-reader" aria-label="' . esc_attr__( 'Scroll to top', 'gadgeto' ) . '" class="thebase-scroll-to-top scroll-up-wrap scroll-ignore scroll-up-side-' . esc_attr( thebase()->option( 'scroll_up_side' ) ) . ' scroll-up-style-' . esc_attr( thebase()->option( 'scroll_up_style' ) ) . ' vs-lg-' . ( thebase()->sub_option( 'scroll_up_visiblity', 'desktop' ) ? 'true' : 'false' ) . ' vs-md-' . ( thebase()->sub_option( 'scroll_up_visiblity', 'tablet' ) ? 'true' : 'false' ) . ' vs-sm-' . ( thebase()->sub_option( 'scroll_up_visiblity', 'mobile' ) ? 'true' : 'false' ) . '">';
		thebase()->print_icon( thebase()->option( 'scroll_up_icon' ), esc_attr__( 'Scroll to top', 'gadgeto' ), false );
		echo '</button>';
	}
}
