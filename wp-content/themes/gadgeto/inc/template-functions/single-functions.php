...
function get_related_posts_args( $post_id ) {
	if ( apply_filters( 'thebase_related_posts_use_tags', true ) ) {
		// Get categories.
	...
	return apply_filters( 'thebase_related_posts_args', $related_args );
}
...
function get_related_posts_columns() {
	if ( thebase()->option( 'post_related_columns' ) ) {
		if ( thebase()->option( 'post_related_columns' ) == 2 ) {
			$cols = array(
				'xxl' => 2,
				'xl'  => 2,
				'md'  => 2,
		...
		} else if ( thebase()->option( 'post_related_columns' ) == 4 ) {
			$cols = array(
				'xxl' => 4,
				'xl'  => 4,
				'md'  => 4,
	...
	return apply_filters( 'thebase_related_posts_carousel_columns', $cols );
}
