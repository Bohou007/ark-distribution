...
class TheBase_CSS {
	...
	public function render_font_family( $font, $area = 'headers' ) {
		...
		return apply_filters( 'thebase_theme_font_family_string', $font_string );
	}
	/**
	...
	public function render_font_no_color( $font, $css, $inherit = null ) {
		...
		if ( ! empty( $family ) ) {
			...
			$css->add_property( 'font-family', apply_filters( 'thebase_theme_font_family_string', $family ) );
			if ( isset( $font['google'] ) && true === $font['google'] ) {
				if ( ! empty( $inherit ) && 'body' === $inherit ) {
	...
	public function maybe_add_google_font( $font, $full = null ) {
		...
		} elseif ( ! empty( $full ) && 'body' === $full && 'inherit' !== thebase()->sub_option( 'heading_font', 'family' ) ) {
			$new_variant = array( $font['variant'], '700' );
		}
		if ( ! empty( $full ) && 'body' === $full ) {
			if ( thebase()->option( 'load_base_italic' ) ) {
