...
class TheBase_Dashboard_Settings {
	...
	public function add_menu() {
		$page = add_theme_page( __( 'gadgeto - Next Generation Theme', 'gadgeto' ), __( 'gadgeto', 'gadgeto' ), apply_filters( 'thebase_admin_settings_capability', 'manage_options' ), 'gadgeto', array( $this, 'config_page' ) );
	...
	public function scripts() {
		...
		$button_label = esc_html__( 'Browse WP Starter Templates', 'gadgeto' );
		$data_action  = '';
		if ( ! defined( 'WP_STARTER_TEMPLATES_VERSION' ) ) {
			if ( ! isset( $installed_plugins['wp-starter-templates/wp-starter-templates.php'] ) ) {
				$button_label = esc_html__( 'Install WP Starter Templates', 'gadgeto' );
				$data_action  = 'install';
			} elseif ( ! self::active_plugin_check( 'wp-starter-templates/wp-starter-templates.php' ) ) {
				$button_label = esc_html__( 'Activate WP Starter Templates', 'gadgeto' );
				$data_action  = 'activate';
			}
		}
		wp_enqueue_style( 'thebase-dashboard', get_template_directory_uri() . '/inc/dashboard/react/dash-controls.min.css', array( 'wp-components' ), gadgeto_VERSION );
		wp_enqueue_script( 'thebase-dashboard', get_template_directory_uri() . '/assets/js/dashboard.js', array( 'wp-i18n', 'wp-element', 'wp-plugins', 'wp-components', 'wp-api', 'wp-hooks', 'wp-edit-post', 'lodash', 'wp-block-library', 'wp-block-editor', 'wp-editor', 'jquery' ), gadgeto_VERSION, true );
		wp_localize_script(
			'thebase-dashboard',
	...
	public function load_api_settings() {
		...
		register_setting(
			...
			array(
				'type'              => 'string',
				'description'       => __( 'Config Modules', 'gadgeto' ),
		...
		<div class="thebase_theme_dash_head">
			<div class="thebase_theme_dash_head_container">
				<div class="thebase_theme_dash_logo">
					<img src="<?php echo esc_attr( get_template_directory_uri() . '/assets/images/gadgeto-logo.png' ); ?>">
				</div>
				<div class="thebase_theme_dash_version">
					<span>
						<?php echo esc_html( gadgeto_VERSION ); ?>
					</span>
				</div>
			</div>
		</div>
		<div class="wrap thebase_theme_dash">
			<div class="thebase_theme_dashboard">
				<h2 class="notices" style="display:none;"></h2>
				<?php settings_errors(); ?>
				<div class="page-grid">
					<div class="thebase_theme_dashboard_main">
					</div>
					<div class="side-panel">
						<?php do_action( 'thebase_theme_dash_side_panel' ); ?>
						<div class="community-section sidebar-section components-panel">
							<div class="components-panel__body is-opened">
								<h2><?php esc_html_e( 'Web Creators Community', 'gadgeto' ); ?></h2>
								<p><?php esc_html_e( 'Join our community of fellow thebase users creating effective websites! Share your site, ask a question and help others.', 'gadgeto' ); ?></p>
								<a href="https://www.facebook.com/groups/webcreatorcommunity" target="_blank" class="sidebar-link"><?php esc_html_e( 'Join our Facebook Group', 'gadgeto' ); ?></a>
							</div>
						</div>
						<div class="support-section sidebar-section components-panel">
							<div class="components-panel__body is-opened">
								<h2><?php esc_html_e( 'Video Tutorials', 'gadgeto' ); ?></h2>
								<p><?php esc_html_e( 'Want a guide? We have video tutorials to walk you through getting started.', 'gadgeto' ); ?></p>
								<a href="https://basetheme.net/learn-thebase/" target="_blank" class="sidebar-link"><?php esc_html_e( 'Watch Videos', 'gadgeto' ); ?></a>
							</div>
						</div>
						<div class="support-section sidebar-section components-panel">
							<div class="components-panel__body is-opened">
								<h2><?php esc_html_e( 'Documentation', 'gadgeto' ); ?></h2>
								<p><?php esc_html_e( 'Need help? We have a knowledge base full of articles to get you started.', 'gadgeto' ); ?></p>
								<a href="https://basetheme.net/knowledge-base/" target="_blank" class="sidebar-link"><?php esc_html_e( 'Browse Docs', 'gadgeto' ); ?></a>
							</div>
						</div>
						<div class="support-section sidebar-section components-panel">
							<div class="components-panel__body is-opened">
								<h2><?php esc_html_e( 'Support', 'gadgeto' ); ?></h2>
								<p><?php esc_html_e( 'Have a question, we are happy to help! Get in touch with our support team.', 'gadgeto' ); ?></p>
								<a href="https://basetheme.net/free-support/" target="_blank" class="sidebar-link"><?php esc_html_e( 'Submit a Ticket', 'gadgeto' ); ?></a>
							</div>
						</div>
					</div>
				</div>
