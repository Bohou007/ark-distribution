...
class RowComponent extends Component {
	render() {
		...
		let besideItems = [];
		return (
			<div className={ `thebase-builder-areas ${ centerClass }` }>
				<Button
					className="thebase-row-left-actions"
					aria-label={ __( 'Edit Settings for', 'gadgeto' ) + ' ' + ( this.props.row === 'popup' ? __( 'Off Canvas', 'gadgeto' ) : this.props.row + ' ' + __( 'Row', 'gadgeto' ) ) }
					onClick={ () => this.props.focusPanel( mode + '_' + this.props.row ) }
					icon="admin-generic"
				>
				</Button>
				<Button
					className="thebase-row-actions"
					aria-label={ __( 'Edit Settings for', 'gadgeto' ) + ' ' + ( this.props.row === 'popup' ? __( 'Off Canvas', 'gadgeto' ) : this.props.row + ' ' + __( 'Row', 'gadgeto' ) ) }
					onClick={ () => this.props.focusPanel( mode + '_' + this.props.row ) }
				>
					{ ( this.props.row === 'popup' ? __( 'Off Canvas', 'gadgeto' ) : this.props.row + ' ' + __( 'Row', 'gadgeto' ) ) }
					<Dashicon icon="admin-generic" />
				</Button>
				<div className="thebase-builder-group thebase-builder-group-horizontal" data-setting={ this.props.row }>
