...
class BuilderComponent extends Component {
	...
	focusPanel( item ) {
		if ( undefined !== this.props.customizer.section( 'thebase_customizer_' + item ) ) {
			this.props.customizer.section( 'thebase_customizer_' + item ).focus();
		}
	}
	focusItem( item ) {
	...
	render() {
		return (
			<div className={ `thebase-control-field thebase-builder-items${ ( this.controlParams.rows.includes( 'popup' ) ? ' thebase-builder-items-with-popup' : '' ) }` }>
				...
				<div className="thebase-builder-row-items">
	...
	// linkFocusButtons() {
	// 	this.props.control.container.on( 'click', '.thebase-builder-areas .thebase-builder-item', function( e ) {
	// 		e.preventDefault();
