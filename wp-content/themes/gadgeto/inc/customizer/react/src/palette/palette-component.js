...
class ColorComponent extends Component {
	constructor(props) {
		...
		let baseDefault =  ( thebaseCustomizerControlsData.palette ? JSON.parse( thebaseCustomizerControlsData.palette ) : { palette: [] } );
	...
	render() {
		...
		return (
				<div className="thebase-control-field thebase-palette-control thebase-color-control">
					<div className="thebase-palette-header">
					...
					<div className="thebase-palette-colors">
					...
					<div className={'thebase-palette-import-wrap'}>
						<Button className={ 'thebase-palette-import' } onClick={ () => { this.state.isVisible ? toggleClose() : toggleVisible() } }>
							<Dashicon icon="portfolio" />
						</Button>
						{ this.state.isVisible && (
							<Popover position="bottom right" className="thebase-palette-popover-copy-paste" onClose={ toggleClose }>
								<TabPanel className="thebase-palette-popover-tabs"
									activeClass="active-tab"
									initialTabName={ 'import'}
									tabs={ [
										{
											name: 'import',
											title: __( 'Select a Color Set', 'gadgeto' ),
											className: 'thebase-color-presets',
										},
										{
											name: 'custom',
											title: __( 'Export/Import', 'gadgeto' ),
											className: 'thebase-export-import',
										}
									] }>
									{
										( tab ) => {
													...
													tabout = (
														<Fragment>
															{ Object.keys( presetPalettes ).map( ( item, index ) => {
																return (
																	<Button
																		className={ 'thebase-palette-item' }
																		style={ {
																			height: '100%',
																			width: '100%',
																		} }
																	...
																	>
																		{ Object.keys( presetPalettes[item] ).map( ( color, subIndex ) => {
																			return (
																				...
																				} } className="thebase-swatche-item-wrap">
																					<span
																						className={ 'thebase-swatch-item' }
																						style={ {
																							height: '100%',
																							display: 'block',
																							width: '100%',
													...
													tabout = (
														<Fragment>
															...
															{ this.state.importError && (
																<p style={{color:'red'}}>{ __( 'Error with Import data', 'gadgeto' ) }</p>
															) }
															<Button
																className={ 'thebase-import-button' }
																isPrimary
					...
					{ this.props.control.params.description && (
						<span className="customize-control-description"><a href="https://basetheme.net/docs/how-to-use-the-thebase-color-palette/" target="_blank">{ this.props.control.params.description }</a></span>
					) }
				</div>
		);
	}
	...
	updateValues( value ) {
		...
		thebaseCustomizerControlsData.palette = JSON.stringify( value );
	}
}
