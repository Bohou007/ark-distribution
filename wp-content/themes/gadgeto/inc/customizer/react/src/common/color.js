...
import TheBaseColorPicker from './color-picker';
...
class ColorControl extends Component {
	...
	render() {
		...
		return (
			<div className="thebase-color-picker-wrap">
				...
				{ showingGradient && (
					<Fragment>
						{ this.state.isVisible && (
							<Popover position={ position } className="thebase-popover-color" onClose={ toggleClose }>
				...
				{ ! showingGradient && (
					<Fragment>
						{ this.state.isVisible && this.state.refresh && (
							<Popover position="top left" className="thebase-popover-color" onClose={ toggleClose }>
								<TheBaseColorPicker
									color={ ( this.state.isPalette && this.state.palette.palette && this.state.palette.palette[parseInt(this.state.color.slice(-1), 10 ) - 1] ? this.state.palette.palette[parseInt(this.state.color.slice(-1), 10 ) - 1 ].color : this.state.color ) }
									onChange={ ( color ) => this.onChangeState( color, '' ) }
									onChangeComplete={ ( color ) => this.onChangeComplete( color, '' ) }
				...
				<Tooltip text={ this.props.tooltip ? this.props.tooltip : __( 'Select Color', 'gadgeto' ) }>
					<div className="color-button-wrap">
						<Button className={ 'thebase-color-icon-indicate' } onClick={ () => { this.state.isVisible ? toggleClose() : toggleVisible() } }>
							<ColorIndicator className="thebase-advanced-color-indicate" colorValue={ ( this.state.isPalette ? 'var(--global-' + this.props.color + ')' : this.props.color ) } />
