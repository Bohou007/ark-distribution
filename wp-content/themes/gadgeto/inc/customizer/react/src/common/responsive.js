...
class ResponsiveControl extends Component {
	...
	render() {
		...
		deviceMap = {
			'desktop': {
				'tooltip': __( 'Desktop', 'gadgeto' ),
				'icon': <Icon icon={ Icons.desktop }/>
			},
			'tablet': {
				'tooltip': __( 'Tablet', 'gadgeto' ),
				'icon': <Icon icon={ Icons.tablet }/>
			},
			'mobile': {
				'tooltip': __( 'Mobile', 'gadgeto' ),
														...
														<Button
																...
																onClick={() => {
																	let event = new CustomEvent(
																			'thebaseChangedRepsonsivePreview', {
																				'detail': device
																			} );
	...
	linkResponsiveButtons() {
		let self = this;
		document.addEventListener( 'thebaseChangedRepsonsivePreview', function(e) {
