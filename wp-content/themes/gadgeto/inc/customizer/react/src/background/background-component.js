...
import TheBaseGradientPicker from './gradient-component.js';
import TheBaseColorPicker from '../common/color-picker';
...
class BackgroundComponent extends Component {
	constructor(props) {
		...
		let defaultParams = {
			...
			repeat: {
				'no-repeat': {
					name: __( 'No Repeat', 'gadgeto' ),
				},
				'repeat': {
					name: __( 'Repeat', 'gadgeto' ),
				},
				'repeat-x': {
					name: __( 'Repeat-X', 'gadgeto' ),
				},
				'repeat-y': {
					name: __( 'Repeat-y', 'gadgeto' ),
				},
			},
			size: {
				auto: {
					name: __( 'Auto', 'gadgeto' ),
				},
				cover: {
					name: __( 'Cover', 'gadgeto' ),
				},
				contain: {
					name: __( 'Contain', 'gadgeto' ),
				},
			},
			attachment: {
				scroll: {
					name: __( 'Scroll', 'gadgeto' ),
				},
				fixed: {
					name: __( 'Fixed', 'gadgeto' ),
				},
			},
			attachments: {
				desktop: {},
		...
		const palette = JSON.parse( this.props.customizer.control( 'thebase_color_palette' ).setting.get() );
		//console.log( palette );
		this.state = {
			value: value,
	...
	render() {
		...
		const toggleVisible = () => {
			const updateColors = JSON.parse( this.props.customizer.control( 'thebase_color_palette' ).setting.get() );
		...
		const controlLabel = (
			<Fragment>
				{ this.state.currentDevice !== 'desktop' && (
					<Tooltip text={ __( 'Reset Device Values', 'gadgeto' ) }>
						<Button
		...
		if( this.state.supportGradient ) {
			tab_options = [
				{
					name: 'color',
					title: __( 'Color', 'gadgeto' ),
					className: 'thebase-color-background',
				},
				{
					name: 'gradient',
					title: __( 'Gradient', 'gadgeto' ),
					className: 'thebase-gradient-background',
				},
				{
					name: 'image',
					title: __( 'Image', 'gadgeto' ),
					className: 'thebase-image-background',
				},
			];
		} else {
			tab_options = [
		...
		return (
			<div className="thebase-control-field thebase-background-control">
				<ResponsiveControl
					onChange={ ( currentDevice) => this.setState( { currentDevice } ) }
					controlLabel={ controlLabel }
				>
					<div className="thebase-background-picker-wrap">
						{ this.state.isVisible && (
							<Popover position="top left" className="thebase-popover-color" onClose={ maybeToggleClose }>
								<TabPanel className="thebase-popover-tabs thebase-background-tabs"
												...
												if ( 'image' === tab.name ) {
													tabout = (
														<Fragment>
															...
															<span className="customize-control-title">{ __( 'Background Repeat', 'gadgeto' ) }</span>
															...
															<span className="customize-control-title">{ __( 'Background Size', 'gadgeto' ) }</span>
															<ButtonGroup className="thebase-radio-container-control">
															...
															<span className="customize-control-title">{ __( 'Background Attachment', 'gadgeto' ) }</span>
															<ButtonGroup className="thebase-radio-container-control">
													...
													tabout = (
														<Fragment>
															{ this.state.isVisible && this.state.refresh && (
																<div className="thebase-background-color-wrap">
																	<TheBaseColorPicker
																		//presetColors={ [] }
						...
						<Tooltip text={ this.controlParams.tooltip ? this.controlParams.tooltip : __( 'Select Background', 'gadgeto' ) }>
							<div className="background-button-wrap">
								<Button className={ 'thebase-background-icon-indicate' } onClick={ () => { this.state.isVisible ? toggleClose() : toggleVisible() } }>
									{ ( undefined === this.state.value[ this.state.currentDevice ] || undefined === this.state.value[ this.state.currentDevice ].type || this.state.value[ this.state.currentDevice ].type === 'color' ) && (
										<Fragment>
											<ColorIndicator className="thebase-advanced-color-indicate" colorValue={ getColorPreviewValue( this.state.currentDevice ) } />
											...
											{ showGradientPreview( this.state.currentDevice ) &&
												<ColorIndicator className="thebase-advanced-color-indicate" colorValue={ getGradientPreviewValue( this.state.currentDevice ) } />
											}
										</Fragment>
									) }
	...
	preventClose() {
		let self = this;
		document.addEventListener( 'thebaseOpenMediaModal', function(e) {
