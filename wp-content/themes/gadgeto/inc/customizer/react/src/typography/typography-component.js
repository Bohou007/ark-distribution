...
import TheBaseWebfontLoader from '../common/font-loader.js';
...
class TypographyComponent extends Component {
	constructor() {
		...
		this.state = {
			...
			activeFont: [],
			value: value,
			fontVars: ( thebaseCustomizerControlsData.gfontvars ? thebaseCustomizerControlsData.gfontvars : [] ),
			customFontVars: ( thebaseCustomizerControlsData.cfontvars ? thebaseCustomizerControlsData.cfontvars : [] ),
		};
	...
	render() {
		...
		const fontFamilyTab = (
			<Fragment>
				<div className="thebase-font-family-search">
					<TextControl
						type="text"
					...
					{ undefined !== this.state.search && '' !== this.state.search && (
						<Button
							className="thebase-clear-search"
							onClick={ () => {
				...
				<div className="thebase-font-family-list-wrapper">
					<ButtonGroup id={ this.controlParams.id } className="thebase-font-family-list" aria-label={ __( 'Font Family List' ) }>
		...
		const fontStyleTab = (
			<Fragment>
				<div className="thebase-font-variant-list-wrapper">
					<ButtonGroup id={ this.controlParams.id + '-style' } className="thebase-font-variant-list" aria-label={ __( 'Font Style List' ) }>
		...
		const fontSizeTab = (
			<Fragment>
				<div class="thebase-range-control">
					<ResponsiveControl
						onChange={ ( currentDevice) => this.setState( { currentDevice } ) }
						controlLabel={ __( 'Font Size', 'gadgeto' ) }
						tooltip={ false }
					>
						<RangeControl
						...
						{ this.controlParams.sizeUnits && (
							<div className="thebase-select-units">
				...
				<div class="thebase-range-control">
					<ResponsiveControl
						onChange={ ( currentDevice) => this.setState( { currentDevice } ) }
						controlLabel={ __( 'Line Height', 'gadgeto' ) }
						tooltip={ false }
					>
						<RangeControl
				...
				<div class="thebase-range-control">
					<ResponsiveControl
						onChange={ ( currentDevice) => this.setState( { currentDevice } ) }
						controlLabel={ __( 'Letter Spacing', 'gadgeto' ) }
						tooltip={ false }
					>
						<RangeControl
				...
				<div class="thebase-range-control thebase-transform-controls">
					<span className="customize-control-title">{ __( 'Transform' ) }</span>
					<ButtonGroup className="thebase-radio-container-control thebase-radio-icon-container-control">
			...
			<Fragment>
				{
					...
					<span className="customize-control-title">
						<Tooltip text={ __( 'Reset Values', 'gadgeto' ) }>
							<Button
		...
		return (
			<div className="thebase-control-field thebase-typography-control-wrap">
				<div className="thebase-typography-control">
					{ controlLabel }
					<div className="thebase-typography-controls">
						{ this.state.isVisible && 'family' === this.controlParams.options && (
							<Popover position="top right" className="thebase-popover-color thebase-popover-typography" onClose={ toggleClose }>
								<div className="thebase-popover-typography-single-item">
									{ fontFamilyTab }
								</div>
							</Popover>
						) }
						{ this.state.isVisible && ( 'all' === this.controlParams.options || 'no-color' === this.controlParams.options ) && (
							<Popover position="top left" className="thebase-popover-color thebase-popover-typography" onClose={ toggleClose }>
								<TabPanel className="thebase-popover-tabs thebase-typography-tabs thebase-background-tabs"
									...
									tabs={ [
										{
											name: 'font',
											title: __( 'Font', 'gadgeto' ),
											className: 'thebase-font-typography',
										},
										{
											name: 'style',
											title: __( 'Style', 'gadgeto' ),
											className: 'thebase-style-typography',
										},
										{
											name: 'size',
											title: __( 'Size', 'gadgeto' ),
											className: 'thebase-size-typography',
										},
									] }>
									{
										( tab ) => {
						...
						{ ( 'all' === this.controlParams.options || 'family' === this.controlParams.options || 'no-color' === this.controlParams.options ) && (
							<Tooltip text={ this.controlParams.tooltip ? this.controlParams.tooltip : __( 'Select Font', 'gadgeto' ) }>
								<div className="typography-button-wrap">
									<Button className={ 'thebase-typography-family-indicate' } onClick={ () => { this.state.isVisible ? toggleClose() : toggleVisible('font') } }>
						...
						{ ( 'all' === this.controlParams.options || 'no-color' === this.controlParams.options ) && (
							<Tooltip text={ this.controlParams.tooltip ? this.controlParams.tooltip : __( 'Select Style', 'gadgeto' ) }>
								<div className="typography-button-wrap">
									<Button className={ 'thebase-typography-style-indicate' } onClick={ () => { this.state.isVisible ? toggleClose() : toggleVisible('style') } }>
										{ this.state.value.variant ? this.state.value.variant : __( 'inherit', 'gadgeto' ) }
									</Button>
								</div>
							</Tooltip>
						) }
						{ ( 'all' === this.controlParams.options || 'size' === this.controlParams.options || 'no-color' === this.controlParams.options ) && (
							<Tooltip text={ this.controlParams.tooltip ? this.controlParams.tooltip : __( 'Select Size', 'gadgeto' ) }>
								<div className="typography-button-wrap">
									<Button className={ 'thebase-typography-size-indicate' } onClick={ () => { this.state.isVisible ? toggleClose() : toggleVisible('size') } }>
										{ currentSize ? currentSize + this.state.value.sizeType : __( 'inherit', 'gadgeto' ) }
									</Button>
								</div>
							</Tooltip>
						) }
						{ ( 'all' === this.controlParams.options || 'family' === this.controlParams.options || 'no-color' === this.controlParams.options ) && (
							<Tooltip text={ __( 'Show Preview Text', 'gadgeto' ) }>
								<div className="typography-button-wrap">
									<Button className={ 'thebase-typography-preview-indicate' } onClick={ () => { this.state.isPreviewVisible ? toggleClosePreview() : toggleVisiblePreview() } }>
				...
				{ this.state.value.google && (
					<TheBaseWebfontLoader config={ configVariants } >
					</TheBaseWebfontLoader>
				) }
				{ this.state.isPreviewVisible && (
					<div className="thebase-preview-font" style={ { 
						...
						{ __( 'Design is not just what it looks like and feels like. Design is how it works.', 'gadgeto' ) }
					</div>
				) }
			</div>
		);
	...
	linkRemoteUpdate() {
		if ( 'base_font' === this.controlParams.id || 'heading_font' === this.controlParams.id ) {
			let self = this;
			document.addEventListener( 'thebaseRemoteUpdateFonts', function( e ) {
