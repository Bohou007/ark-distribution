...
class FontPairModal extends Component {
	constructor() {
		...
		this.state = {
			isVisible: false,
			confirm: '',
			fonts: ( thebaseCustomizerControlsData.fontPairs ? thebaseCustomizerControlsData.fontPairs : [] ),
		};
	}
	render() {
		const toggleVisible = () => {
		...
		return (
			<div className={'thebase-font-pair-wrap'}>
				<Button className={ 'thebase-font-pair-btn' } label={ __( 'Choose a font', 'gadgeto' ) } onClick={ () => { this.state.isVisible ? toggleClose() : toggleVisible() } }>
					<Dashicon icon="portfolio" />
				</Button>
				{ this.state.isVisible && (
					<Popover position="bottom left" className="thebase-font-pair-popover" onClose={ toggleClose }>
						<h2 style={{ textAlign:'center' }}>{ __( 'Select a Font Pairing', 'gadgeto' ) }</h2>
						<ButtonGroup className="tb-font-pair-group" aria-label={ __( 'Select a Font Pair', 'gadgeto' ) }>
							{ map( this.state.fonts, ( { hfont, bfont, hv, img, name } ) => {
								return (
									this.state.confirm === name ? (
										<Button
											className={ 'tb-font-pair-btn state-confirm' }
											onClick={ () => {
												this.updateSettings( hfont, bfont, hv );
											} }
										>
											{ __( 'Confirm Change Settings?', 'gadgeto' ) }
										</Button>
									) : (
										<Button
											className={ 'tb-font-pair-btn' }
											onClick={ () => {
	...
	updateValues( bodyFont, headingFont ) {
		...
		var event = new CustomEvent('thebaseRemoteUpdateFonts', {
			'detail': 'typography'
		});
