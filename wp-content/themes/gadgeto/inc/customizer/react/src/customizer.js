( function( $, api ) {
	...
	wp.customize.bind( 'ready', function() {
		$( 'input[name=thebase-flush-local-fonts-button]' ).on( 'click', function( e ) {
			var data = {
				wp_customize: 'on',
				action: 'thebase_flush_fonts_folder',
				nonce: thebaseCustomizerControlsData.flushFonts
			};	
			$( 'input[name=thebase-flush-local-fonts-button]' ).attr('disabled', 'disabled');
	
			$.post( ajaxurl, data, function ( response ) {
				if ( response && response.success ) {
					$( 'input[name=thebase-flush-local-fonts-button]' ).val( 'Successfully Flushed' );
				} else {
					$( 'input[name=thebase-flush-local-fonts-button]' ).val( 'Failed, Reload Page and Try Again' );
				}
			});
		});

		wp.customize.state.create( 'thebaseTab' );
		wp.customize.state( 'thebaseTab' ).set( 'general' );
		

		// Set handler when custom responsive toggle is clicked.
		$( '#customize-theme-controls' ).on( 'click', '.thebase-build-tabs-button:not(.thebase-nav-tabs-button)', function( e ) {
			e.preventDefault();

			wp.customize.previewedDevice.set( $( this ).attr( 'data-device' ) );
		});
		// Set handler when custom responsive toggle is clicked.
		$( '#customize-theme-controls' ).on( 'click', '.thebase-compontent-tabs-button:not(.thebase-nav-tabs-button)', function( e ) {
			e.preventDefault();

			wp.customize.state( 'thebaseTab' ).set( $( this ).attr( 'data-tab' ) );
		});
		var setCustomTabElementsDisplay = function() {
			var tabState = wp.customize.state( 'thebaseTab' ).get(),
			$tabs = $( '.thebase-compontent-tabs-button:not(.thebase-nav-tabs-button)' );
			$tabs.removeClass( 'nav-tab-active' ).filter( '.thebase-' + tabState + '-tab' ).addClass( 'nav-tab-active' );
		}
		// Refresh all responsive elements when previewedDevice is changed.
		wp.customize.state( 'thebaseTab' ).bind( setCustomTabElementsDisplay );

		$( '#customize-theme-controls' ).on( 'click', 'customize-section-back', function( e ) {
			wp.customize.state( 'thebaseTab' ).set( 'general' );
		});
		if ( thebaseCustomizerControlsData && thebaseCustomizerControlsData.contexts ) {
			/**
			 * Active callback script (JS version)
			 * ref: https://make.xwp.co/2016/07/24/dependently-contextual-customizer-controls/
			 */
			_.each( thebaseCustomizerControlsData.contexts, function( rules, key ) {
				var getSetting = function( settingName ) {
					...
					switch ( settingName ) {
						case '__device':
							...
							break;
						case '__current_tab':
							return wp.customize.state( 'thebaseTab' );
							break;
						default:
							// Check if we have an extra source in the mix
							if ( thebaseCustomizerControlsData.source ) {
								// Check if this setting might be powered by the extra source.
								if ( wp.customize( thebaseCustomizerControlsData.source + '[' + settingName + ']' ) ) {
									return wp.customize( thebaseCustomizerControlsData.source + '[' + settingName + ']' );
								} else {
									...
									case 'load_italic':
										...
										if ( currentValue['family'] && currentValue['google'] && currentValue['variant'] ) {
											if ( 0 > currentValue['variant'].indexOf( 'italic' ) ) {
												if ( thebaseCustomizerControlsData.gfontvars && thebaseCustomizerControlsData.gfontvars[ currentValue['family'] ] && thebaseCustomizerControlsData.gfontvars[ currentValue['family'] ].v && thebaseCustomizerControlsData.gfontvars[ currentValue['family'] ].v.includes( 'italic' ) ) {
													result = true;
												}
											}
										}
				...
				if ( 0 == key.indexOf( 'thebase_customizer' ) ) {
		...
		var setCustomResponsiveElementsDisplay = function() {
			var device = wp.customize.previewedDevice.get(),
			    $tabs = $( '.thebase-build-tabs-button.nav-tab' );
		...
		var resizePreviewer = function() {
			var $section = $( '.control-section.thebase-builder-active' );
			var $footer = $( '.control-section.thebase-footer-builder-active' );
			if ( $body.hasClass( 'thebase-builder-is-active' ) || $body.hasClass( 'thebase-footer-builder-is-active' ) ) {
				if ( $body.hasClass( 'thebase-footer-builder-is-active' ) && 0 < $footer.length && ! $footer.hasClass( 'thebase-builder-hide' ) ) {
					wp.customize.previewer.container.css( 'bottom', $footer.outerHeight() + 'px' );
				} else if ( $body.hasClass( 'thebase-builder-is-active' ) && 0 < $section.length && ! $section.hasClass( 'thebase-builder-hide' ) ) {
		...
		var initHeaderBuilderPanel = function( panel ) {
			var section =  wp.customize.section( 'thebase_customizer_header_builder' );
			if ( section ) {
				var $section = section.contentContainer,
				section_layout =  wp.customize.section( 'thebase_customizer_header_layout' );
				...
				panel.expanded.bind(function( isExpanded ) {
					...
					if ( isExpanded ) {
						$body.addClass( 'thebase-builder-is-active' );
						$section.addClass( 'thebase-builder-active' );
						$section.css('display', 'none').height();
						$section.css('display', 'block');
					} else {
						$body.removeClass( 'thebase-builder-is-active' );
						$section.removeClass( 'thebase-builder-active' );
					}
				...
				$section.on( 'click', '.thebase-builder-tab-toggle', function( e ) {
					e.preventDefault();
					$section.toggleClass( 'thebase-builder-hide' );
					resizePreviewer();
				});
			}

		};
		wp.customize.panel( 'thebase_customizer_header', initHeaderBuilderPanel );
		/**
		 * Init Header & Footer Builder
		 */
		var initFooterBuilderPanel = function( panel ) {
			var section =  wp.customize.section( 'thebase_customizer_footer_builder' );
			if ( section ) {
				var $section = section.contentContainer,
				section_layout =  wp.customize.section( 'thebase_customizer_footer_layout' );
				// If Header panel is expanded, add class to the body tag (for CSS styling).
				panel.expanded.bind(function( isExpanded ) {
					_.each(section.controls(), function( control ) {
						if ( 'resolved' === control.deferred.embedded.state() ) {
					...
					if ( isExpanded ) {
						$body.addClass( 'thebase-footer-builder-is-active' );
						$section.addClass( 'thebase-footer-builder-active' );
						$section.css('display', 'none').height();
						$section.css('display', 'block');
					} else {
						$body.removeClass( 'thebase-footer-builder-is-active' );
						$section.removeClass( 'thebase-footer-builder-active' );
					}
					_.each(section_layout.controls(), function( control ) {
						if ( 'resolved' === control.deferred.embedded.state() ) {
							return;
		...
		wp.customize.panel( 'thebase_customizer_footer', initFooterBuilderPanel );
	});
