...
class Customizer_Sanitize {
	 ...
	 * @return string
	 */
	public static function thebase_sanitize_key_string( $value ) {
	 ...
	 * @return mixed
	 */
	public static function thebase_sanitize_option( $start_value ) {
		if ( empty( $start_value ) ) {
			return '';
		}
		if ( ! is_array( $start_value ) ) {
			return self::thebase_sanitize_key_string( $start_value );
		}
		$new_value = array();
		foreach ( $start_value as $key => $value ) {
			// Sanitize key, remove spaces and special characters.
			$key = self::thebase_sanitize_key_string( $key );
			if ( is_array( $value ) ) {
				$new_value[ $key ] = self::thebase_sanitize_option( $value );
			} else {
				switch ( $key ) {
					case 'family':
					case 'label':
	 ...
	 * @return bool
	 */
	public static function thebase_sanitize_toggle( $value ) {
	 ...
	 * @return mixed
	 */
	public static function thebase_sanitize_google_subsets( $start_value ) {
		if ( empty( $start_value ) ) {
			return '';
		}
		if ( ! is_array( $start_value ) ) {
