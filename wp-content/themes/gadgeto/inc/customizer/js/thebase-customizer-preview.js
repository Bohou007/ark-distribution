( function( $ ) {
	...
	document.addEventListener('DOMContentLoaded', function () {
		...
		if ( hasSelectiveRefresh ) {
			wp.customize.selectiveRefresh.bind('partial-content-rendered', function (placement) {
				...
				if ( typeof window.thebase.initStickyHeader !== "undefined" ) { 
	...
	var thebaseCustomizer = {
		live_css_typography : function( key, rules, newValue ) {
			var styleID = 'thebase-customize-preview-css-' + key,
			    $style = $( '#' + styleID ),
				css = '',
	...
	if ( thebaseCustomizerPreviewData && thebaseCustomizerPreviewData.liveControl ) {
		_.each( thebaseCustomizerPreviewData.liveControl, function( liveControl, key ) {
			var eachLiveControl = {};
			...
			wp.customize( key, function( value ) {
				value.bind( function( newValue ) {
					_.each( eachLiveControl, function( rule, type ) {
						...
						thebaseCustomizer[ functionName ]( key, rule, newValue );
					});
				});
			});
		});
