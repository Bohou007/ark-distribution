...
class Theme {
	...
	/**
	 * Main TheBase\thebase Class Instance.
	 *
	 * Insures that only one instance of TheBase Class exists in memory at any one
	 * time. Also prevents needing to define globals all over the place.
	 *
	 * @static
	 * @static var array $instance
	 *
	 * @return TheBase\thebase Class
	 */
			...
			if ( ! $component instanceof Component_Interface ) {
				throw new InvalidArgumentException(
					sprintf(
						...
						__( 'The theme component %1$s does not implement the %2$s interface.', 'gadgeto' ),
						gettype( $component ),
	...
	public function __clone() {
		...
		_doing_it_wrong( __FUNCTION__, esc_html__( 'Something went wrong.', 'gadgeto' ), '1.0' );
	}
	 ...
	 *
	 * Calling `thebase()` is a short-hand for calling this method on the main theme instance. The instance then allows
	 * for actual template tag methods to be called. For example, if there is a template tag called `posted_on`, it can
	 * be accessed via `thebase()->posted_on()`.
	 *
	...
	public function component( string $slug ) : Component_Interface {
		if ( ! isset( $this->components[ $slug ] ) ) {
			throw new InvalidArgumentException(
				sprintf(
					/* translators: %s: slug */
					__( 'No theme component with the slug %s exists.', 'gadgeto' ),
					$slug
				)
			);
		}
	...
	protected function get_default_components() : array {
		...
		if ( class_exists( 'BBPress' ) && apply_filters( 'thebase_theme_enable_bbpress_component', true ) ) {
