	...
	window.thebase = {
		...
		initNavToggleSubmenus: function() {
			...
			for ( let i = 0; i < navTOGGLE.length; i++ ) {
				window.thebase.initEachNavToggleSubmenu( navTOGGLE[ i ] );
				window.thebase.initEachNavToggleSubmenuInside( navTOGGLE[ i ] );
			}
		},
		initEachNavToggleSubmenu: function( nav ) {
			...
			for ( let i = 0; i < SUBMENUS.length; i++ ) {
				...
				if ( dropdown ) {
					...
					dropdownBtn.setAttribute( 'aria-label', thebaseConfig.screenReader.expand );
					...
					dropdownBtn.addEventListener( 'click', function( e ) {
						e.preventDefault();
						window.thebase.toggleSubMenu( e.target.parentNode );
					} );
		
					// Clean up the toggle if a mouse takes over from keyboard.
					parentMenuItem.addEventListener( 'mouseleave', function( e ) {
						window.thebase.toggleSubMenu( e.target, false );
					} );
					...
					parentMenuItem.querySelector( 'a' ).addEventListener( 'focus', function( e ) {
						...
						for ( let j = 0; j < parentMenuItemsToggled.length; j++ ) {
							window.thebase.toggleSubMenu( parentMenuItemsToggled[ j ], false );
						}
					} );
					...
					SUBMENUS[ i ].addEventListener( 'keydown', function( e ) {
						...
						if ( 9 === e.keyCode ) {
							if ( e.shiftKey ) {
								...
								if ( window.thebase.isfirstFocusableElement (SUBMENUS[ i ], document.activeElement, focusSelector ) ) {
									window.thebase.toggleSubMenu( SUBMENUS[ i ].parentNode, false );
								}
								// Means we're tabbing out of the end of the submenu.
							} else if ( window.thebase.islastFocusableElement( SUBMENUS[ i ], document.activeElement, focusSelector ) ) {
								window.thebase.toggleSubMenu( SUBMENUS[ i ].parentNode, false );
							}
						}
					} );
			...
			for ( let i = 0; i < SUBMENUPARENTS.length; i++ ) {		
				...
				SUBMENUPARENTS[ i ].addEventListener( 'mouseenter', function( e ) {
					if ( SUBMENUPARENTS[ i ].querySelector( 'ul.sub-menu' ) ) {
						...
						var off = window.thebase.getOffset( elm );
						var l = off.left;
		...
		toggleSubMenu: function( parentMenuItem, forceToggle ) {
			...
			*/
			if ( parentMenuItemToggled ) {
				...
				toggleButton.setAttribute( 'aria-label', thebaseConfig.screenReader.expand );
				...
				for ( let i = 0; i < subMenuItemsToggled.length; i++ ) {
					window.thebase.toggleSubMenu( subMenuItemsToggled[ i ], false );
				}
			} else {
				...
				for ( let i = 0; i < parentMenuItemsToggled.length; i++ ) {
					window.thebase.toggleSubMenu( parentMenuItemsToggled[ i ], false );
				}

				// Toggle "on" the submenu.
				parentMenuItem.classList.add( 'menu-item--toggled-on' );
				subMenu.classList.add( 'toggle-show' );
				toggleButton.setAttribute( 'aria-label', thebaseConfig.screenReader.collapse );
			}
		},
		/**
		...
		toggleDrawer: function( element, changeFocus ) {
			...
			window.thebase.toggleAttribute( toggle, 'aria-expanded', 'true', 'false' );
					...
					if ( toggle.dataset.setFocus && changeFocus ) {
						...
						if ( focusElement ) {
							...
							if ( focusElement.hasAttribute( 'aria-expanded') ) {
								window.thebase.toggleAttribute( focusElement, 'aria-expanded', 'true', 'false' );
							}
						}
					}
				}, duration);
		...
		initToggleDrawer: function() {
			...
			for ( let i = 0; i < drawerTOGGLE.length; i++ ) {
				drawerTOGGLE[ i ].addEventListener('click', function( event ) {
					...
					window.thebase.toggleDrawer( drawerTOGGLE[ i ] );
				} );
			}
				...
				if ( event.keyCode === 27 ) {
					if ( document.querySelectorAll( '.popup-drawer.show-drawer.active' ) ) {
						...
						document.querySelectorAll( '.popup-drawer.show-drawer.active' ).forEach(function ( element ) {
							window.thebase.toggleDrawer( document.querySelector('*[data-toggle-target="' + element.dataset.drawerTargetString + '"]' ) );
						} );
					}
				}
			  });
			...
			document.addEventListener( 'click', function (event) {
				...
				if ( target === modal ) {
					window.thebase.toggleDrawer(document.querySelector('*[data-toggle-target="' + modal.dataset.drawerTargetString + '"]'));
				}
		...
		initMobileToggleSub: function() {
			...
			modalMenus.forEach( function( modalMenu ) {
				...
				if ( activeMenuItem ) {
					window.thebase.findParents( activeMenuItem, 'li' ).forEach( function( element ) {
						var subMenuToggle = element.querySelector( '.drawer-sub-toggle' );
						if ( subMenuToggle ) {
							window.thebase.toggleDrawer( subMenuToggle, true );
						}
					} );
				}
			} );
			...
			for ( let i = 0; i < drawerSubTOGGLE.length; i++ ) {
				drawerSubTOGGLE[ i ].addEventListener('click', function (event) {
					event.preventDefault();
					window.thebase.toggleDrawer( drawerSubTOGGLE[ i ] );
				} );
			}
		},
		/**
		...
		initMobileToggleAnchor: function() {
			...
			for ( let i = 0; i < menuLink.length; i++ ) {
				menuLink[ i ].addEventListener('click', function (event) {
					window.thebase.toggleDrawer( mobileModal.querySelector( '.menu-toggle-close' ), false );
				} );
			}
		},
		/**
		...
		initTransHeaderPadding: function() {
			...
			var updateHeroPadding = function( e ) {
				header
				if ( thebaseConfig.breakPoints.desktop <= window.innerWidth ) {
		...
		initStickyHeader: function() {
			var desktopSticky = document.querySelector( '#main-header .thebase-sticky-header' ),
				mobileSticky  = document.querySelector( '#mobile-header .thebase-sticky-header' ),
				wrapper = document.getElementById( 'wrapper' ),
				proSticky = document.querySelectorAll( '.thebase-pro-fixed-above' ),
				proElements = document.querySelectorAll( '.thebase-before-wrapper-item' ),
				activeSize = 'mobile',
				lastScrollTop = 0,
				activeOffsetTop = 0;
				if ( thebaseConfig.breakPoints.desktop <= window.innerWidth ) {
				...
				var updateSticky = function( e ) {
					...
					if ( proElements.length ) {
						...
						offsetTop = window.thebase.getOffset( wrapper ).top - proElementOffset;
					}
					if ( proSticky.length ) {
						var proOffset = 0;
						...
						offsetTop = window.thebase.getOffset( wrapper ).top + proOffset;
					}
					if ( thebaseConfig.breakPoints.desktop <= window.innerWidth ) {
					...
					if ( thebaseConfig.breakPoints.desktop <= window.innerWidth ) {
						if ( activeSize === 'mobile' ) {
							activeOffsetTop = window.thebase.getOffset( activeHeader ).top;
							activeSize = 'desktop';
					...
					if ( 'true' === shrink ) {
						...
						if ( shrinkHeight ) {
							...
							var customShrinkLogo = activeHeader.querySelector( '.thebase-sticky-logo' );
				...
				if ( desktopSticky || mobileSticky ) {
					...
					if ( document.body.classList.contains( 'woocommerce-demo-store' ) && document.body.classList.contains( 'thebase-store-notice-placement-above' ) ) {
		...
		getTopOffset: function() {
			var desktopSticky = document.querySelector( '#main-header .thebase-sticky-header:not([data-reveal-scroll-up="true"])' ),
				mobileSticky  = document.querySelector( '#mobile-header .thebase-sticky-header:not([data-reveal-scroll-up="true"])' ),
		...
		scrollToElement: function( element, history ) {
			...
			var offsetSticky = window.thebase.getTopOffset();
			...
			var checkIfDone = setInterval( function() {
				...
				if ( ( Math.floor( element.getBoundingClientRect().top ) - offsetSticky === 0 ) || atBottom ) {
					element.tabIndex = '-1';
					element.focus();
					if ( element.classList.contains( 'tb-title-item' ) ) {
		...
		anchorScrollToCheck: function( e, respond ) {
			...
			e.preventDefault();
			window.thebase.scrollToElement( targetAnchor );
		},
		/**
			...
			if ( window.location.hash != '' ) {
				...
				if ( element ) {
					window.setTimeout( function() {
						window.thebase.scrollToElement( element, false );
					}, 100 );
				}
			}
			...
			foundLinks.forEach( function( element ) {
				...
				if ( targetURL.pathname === window.location.pathname ) {
					element.addEventListener( 'click', function( e ) {
						window.thebase.anchorScrollToCheck( e );
					} );
				}
			} );
		},
		...
		initScrollToTop: function() {
			var scrollBtn = document.getElementById( "tb-scroll-up" );
			if ( scrollBtn ) {
			...
			var scrollBtnReader = document.getElementById( "tb-scroll-up-reader" );
			if ( scrollBtnReader ) {
		...
		init: function() {
			window.thebase.initNavToggleSubmenus();
			window.thebase.initToggleDrawer();
			window.thebase.initMobileToggleAnchor();
			window.thebase.initMobileToggleSub();
			window.thebase.initOutlineToggle();
			window.thebase.initStickyHeader();
			window.thebase.initStickySidebar();
			window.thebase.initStickySidebarWidget();
			window.thebase.initTransHeaderPadding();
			window.thebase.initAnchorScrollTo();
			window.thebase.initScrollToTop();
		}
	}
	if ( 'loading' === document.readyState ) {
		// The DOM has not yet been loaded.
		document.addEventListener( 'DOMContentLoaded', window.thebase.init );
	} else {
