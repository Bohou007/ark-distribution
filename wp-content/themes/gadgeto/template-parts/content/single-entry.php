	...
	if ( ( 'post' === get_post_type() || get_post_type_object( get_post_type() )->has_archive ) && thebase()->show_post_navigation() ) {
		...
		the_post_navigation(
			apply_filters(
				...
				array(
					'prev_text' => '<div class="post-navigation-sub"><small>' . thebase()->get_icon( 'arrow-left-alt' ) . esc_html__( 'Previous', 'gadgeto' ) . '</small></div>%title',
					'next_text' => '<div class="post-navigation-sub"><small>' . esc_html__( 'Next', 'gadgeto' ) . thebase()->get_icon( 'arrow-right-alt' ) . '</small></div>%title',
				)
			)
		);
		if ( thebase()->option( 'post_footer_area_boxed' ) ) {
