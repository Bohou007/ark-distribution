...
$elements          = thebase()->option( $item_type . '_title_element_meta' );
$title_meta        = array();
$meta_labels       = array();
$meta_divider      = 'dot';
$author_image      = false;
...
if ( isset( $elements ) && is_array( $elements ) ) {
	...
	if ( isset( $elements['author'] ) && $elements['author'] ) {
		...
		if ( isset( $elements['authorEnableLabel'] ) && $elements['authorEnableLabel'] ) {
			$meta_labels['author'] = ( isset( $elements['authorLabel'] ) && ! empty( $elements['authorLabel'] ) ? $elements['authorLabel'] : __( 'By', 'gadgeto' ) );
		}
	...
	if ( isset( $elements['date'] ) && $elements['date'] ) {
		...
		if ( isset( $elements['dateEnableLabel'] ) && $elements['dateEnableLabel'] ) {
			$meta_labels['date'] = ( isset( $elements['dateLabel'] ) && ! empty( $elements['dateLabel'] ) ? $elements['dateLabel'] : __( 'Posted on', 'gadgeto' ) );
		}
	}
	if ( isset( $elements['dateUpdated'] ) && $elements['dateUpdated'] ) {
		...
		if ( isset( $elements['dateUpdatedEnableLabel'] ) && $elements['dateUpdatedEnableLabel'] ) {
			$meta_labels['dateUpdated'] = ( isset( $elements['dateUpdatedLabel'] ) && ! empty( $elements['dateUpdatedLabel'] ) ? $elements['dateUpdatedLabel'] : __( 'Updated on', 'gadgeto' ) );
		}
	}
	if ( isset( $elements['categories'] ) && $elements['categories'] ) {
		...
		if ( isset( $elements['categoriesEnableLabel'] ) && $elements['categoriesEnableLabel'] ) {
			$meta_labels['categories'] = ( isset( $elements['categoriesLabel'] ) && ! empty( $elements['categoriesLabel'] ) ? $elements['categoriesLabel'] : __( 'Posted in', 'gadgeto' ) );
		}
	}
...
<div class="entry-meta entry-meta-divider-<?php echo esc_attr( $meta_divider ); ?>">
	<?php
	do_action( 'thebase_before_entry_meta' );
	foreach ( $title_meta as $title_meta_item ) {
		switch ( $title_meta_item ) {
				...
				if ( post_type_supports( $post_type_obj->name, 'author' ) ) {
					...
					if ( $author_link ) {
						$author_string = sprintf(
							'<span class="author vcard"><a class="url fn n" href="%1$s">'.esc_html__("by ",'gadgeto').'%2$s</a></span>',
							esc_url( get_author_posts_url( $author_id ) ),
							esc_html( get_the_author_meta( 'display_name', $author_id ) )
						);
					} else {
						$author_string = sprintf(
							'<span class="author vcard"><span class="fn n">'.esc_html__("by ",'gadgeto').'%1$s</span></span>',
							esc_html( get_the_author_meta( 'display_name', $author_id ) )
						);
					}
				}
			...
			case 'date':
				...
				if ( ! empty( $time_string ) ) {
					...
					<span class="posted-on">
						...
						echo html_entity_decode( esc_html($time_string)); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
						?>
					</span>
					<?php
				}
			...
			case 'dateUpdated':
				$time_string = sprintf(
					'<span class="entry-date published updated" datetime="%1$s">%2$s</span>',
					esc_attr( get_the_modified_date( 'c' ) ),
					esc_html( get_the_modified_date() )
				);
				if ( ! empty( $time_string ) ) {
			...
			case 'categories':
				if ( 'post' === get_post_type() ) {
					...
					$separator = _x( ', ', 'list item separator', 'gadgeto' );
					?>
			...
			case 'comments':
				if ( thebase()->show_comments() ) {
					...
					if ( '1' === get_comments_number() ) {
						echo esc_html('Comment', 'gadgeto') . ' ' .( get_comments_number());
					} else {
						echo esc_html('Comment', 'gadgeto') . ' ' .(  get_comments_number() );
					}
					echo '</a>';
	...
	do_action( 'thebase_after_entry_meta' );
	?>
</div><!-- .entry-meta -->
<?php
